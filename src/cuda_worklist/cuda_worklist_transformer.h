...
class CudaWorklistTransformer : public CfgVisitor<LatticeType>
{
public:
    std::vector<cuda_worklist::Node> nodes{};
    std::vector<cuda_worklist::Transfer> transfer_functions{};
    std::map<Node<LatticeType>*, int> node_to_index{};
    std::map<std::string, int> variables{};

    CudaWorklistTransformer(std::set<std::string> progVariables){
        int i = 0;
    ...
    void visit_assignment(AssignmentNode<LatticeType>& node) {
        ...
        cuda_worklist::Node& node_struct = nodes.emplace_back();
    ...
    void visit_functionEntry(FunctionEntryNode<LatticeType>& node) { 
        ...
        if(node.arguments.size() >= 1){
            ...
            cuda_worklist::Transfer& last = transfer_functions[node_struct.first_transfer_index];
...
private:
    int add_transfer_function(std::string x, std::set<std::string>& vars){
        ...
        cuda_worklist::Transfer& current_transfer = transfer_functions.emplace_back();
...
template<typename LatticeType>
CudaWorklistTransformer<LatticeType> transform_cuda_worklist(std::vector<std::shared_ptr<Node<LatticeType>>>& nodes) {
    auto variables = get_variables(nodes);
    CudaWorklistTransformer<LatticeType> transformer(variables);
    for(auto &node : nodes){
